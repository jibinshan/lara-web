"use client";
import { NextButton, PrevButton, usePrevNextButtons } from "@/app/(section)/(special)/MenuArrowButtons";
...
const EmblaCarousel: React.FC<PropType> = (props) => {
    ...
    const setTweenNodes = useCallback((emblaApi: EmblaCarouselType): void => {
        tweenNodes.current = emblaApi.slideNodes().map((slideNode) => {
            return slideNode.querySelector(".embla-menu__slide__number") as HTMLElement;
        });
    }, []);
            ...
            <div className="z-50 flex w-full flex-col items-center gap-2">
                    ...
                    <div className="embla-menu__viewport" ref={emblaRef}>
                        ...
                        <div className="flex w-full items-center justify-center pb-6">
                            {slides[selectedIndex]?.name && (
                                <p className="min-h-[80px] max-w-[300px] text-center font-playfair text-4xl text-white">{formatModelName(slides[selectedIndex]?.name)}</p>
                            )}
                        </div>
                    </div>
                </div>
                <div className="relative flex h-auto w-full items-center justify-end gap-2 px-10 md:px-64">
                    ...
                    <p className="flex items-center font-playfair text-3xl">
                        <span className="text-primary">{selectedIndex + 1}</span>/ {scrollSnaps.length}
                    </p>
                </div>
            </div>
