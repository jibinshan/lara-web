...
const MenuItemPopup: FC<MenuItemPopupProps> = ({ children, item }) => {
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="hidden-scrollbar hidden max-h-[625px] max-w-0 flex-col gap-3 overflow-y-scroll border-[1px] border-[#212121] bg-[#070707] p-0 md:flex lg:max-w-[625px]">
                {item.images[0] && <Image src={item.images[0]} width={1980} height={1080} alt={item.name} className="h-auto w-full object-cover md:max-h-[350px]" />}
                <div className="flex flex-col gap-5 px-6 py-6">
                    ...
                    {showNote ? (
                        <div className="flex w-full flex-col gap-2">
                            ...
                            <Textarea id="note" value={note} onChange={(e) => setNote(e.target.value)} rows={3} className="border-none bg-[#0F0F0F]" />
                        </div>
                    ) : (
                        <p
                    ...
                    {GetModifiersFromItemId(item).length > 0 && (
                        <div className="space-y-2">
                            <Label className="text-lg font-semibold text-[#FBEAD2]">Modifiers</Label>
                            {/* max-h-[100px] */}
                            <div className="custom-scrollbar flex h-full max-h-[200px] w-full flex-col gap-2 overflow-y-scroll">
                                {GetModifiersFromItemId(item).map((modifier) => (
                                    ...
                                    >
                                        ...
                                        />
                                        <Label htmlFor={modifier._id} className="flex items-center gap-2 text-[#FBEAD2]">
                                            {modifier.name}
                ...
                {BetaMenuActive && (
                    <DialogFooter className="flex h-fit w-full flex-row justify-end gap-2">
